  ...
  try {
    ...
    const products = await page.evaluate(() => {
      ...
      const productDetails = Array.from(productElements).map(productEl => {
        ...
        const productPriceString = (productEl.querySelector('[itemprop="price"]') as HTMLElement)?.innerText.trim() || '';
        const imagePath = (productEl.querySelector('img[itemprop="image"]') as HTMLImageElement)?.src || '';

        // Clean and convert the price string to a number
        const productPrice = parseFloat(productPriceString.replace(/[^\d.,]/g, '').replace(',', '.'));

        return { name: productName, price: productPrice, imagePath: imagePath };
      });
